...
### Task 4 — Narrow line fitting and subtraction
- **Do:** Fit narrow lines from the mean spec (subtract_narrow_mean), the code will also make mean and rms spec.
- **Deliver:** {target name}_narrow_profile.txt files containing wave/flux from the fitting.
- **Per season:** `[ ]` S1  `[ ]` S2  `[ ]` S3 …

### Task 5 — Measure Hβ flux & 5100Å luminosity (spec_lc pipeline)
...
### Task 6 — Inter-calibrate continuum (pycali)
...
### Task 8 — Line width (FWHM & dispersion)
- **Goal:** Measure Hβ width on mean and rms spectra. 
- **Do:** measure FWHM and σ_line for broad Hβ with narrow subtracted mean or rms spec (measure_linewidth). Per season.
- **Deliver:** FWHM + dispersion table (mean \& rms) per season. Put code/results in `results/season_x/line_width/`
- **Per season:** `[ ]` S1  `[ ]` S2  `[ ]` S3 …

### Task 9 — Black hole mass
...
### Task 11 — Publication figures & tables & Poster draft
